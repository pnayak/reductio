// Ratio tests
describe('Reductio ratio based on column (first)', function() {
  var group;
  ...
  beforeEach(function() {
    ...
    // reducer definition is ratio(evaluate, field, fieldValueforNumerator, fieldForFirst, fieldForSort)
    var reducer = reductio().ratioBoCFirst(true, 'event.correct', true, 'event.segmentId', 'event.completedOn');
    ...
    // reducer definition is ratio(evaluate, field, fieldValueforNumerator, fieldForFirst, fieldForSort)
    var reducer2 = reductio().ratioBoCFirst(true, 'event.correct', true, 'event.segmentId', 'event.segmentId');
  ...
  it('grouping has the right ratio', function() {
    ...
    // console.log("ratioBoCFirst: values = %o", JSON.stringify(values));

    expect(values['zone1'].ratioBoCFirst).toEqual(1.0);
    expect(values['zone2'].ratioBoCFirst).toEqual(0.25);
  });
